...
def total_conv_curves(S_tot, A_tot) :
    ...
    # calculate kmax th_MLE via true bootstrap:
    ...
    def func_log_post(num) :
        ...
        def log_post(theta) :
            ...
            for i in range(s) :
                th = np.zeros((1,2))
                # j = rd.randint(0, num_tot-num_est_max)
                th[0] = theta[i] +0
                pp[i] = stat_functions.log_post_jeff_adapt(th,S_tot[:num],A_tot[:num], Fisher=fisher_approx)
            return pp[:,0]
        return log_post
    ...
    for nn, num in enumerate(num_est_tab) :
        ...
        t_fin, t_tot, acc = stat_functions.adaptative_HM(t0, log_post, pi_log=True, max_iter=iter_HM) #attention: de base HM_k c'est pas ca mais ca revient surement au meme: a voir
        ...
        th_post_jeff_tot_tab[nn,:,0,0] = t_tot[:,0]
        th_post_jeff_tot_tab[nn,:,0,1] = t_tot[:,1]
    ...
    def func_log_post_gamma(num, gam_a=gam_a, gam_b=gam_b, gam_m=gam_m, gam_lamb=gam_lamb) :
        ...
        def log_post(theta) :
            ...
            for i in range(s) :
                ...
                th[0] = theta[i] +0
                # j = rd.randint(0, num_tot-num_est_max)
                log_vrs = stat_functions.log_vrais(S_tot[:num], A_tot[:num], th)
    ...
    for nn, num in enumerate(num_est_tab) :
        ...
        t_fin, t_tot, acc = stat_functions.adaptative_HM(t0, log_post, pi_log=True, max_iter=iter_HM)
        ...
        # th_post_jeff_tot_tab[nn] = t_tot[:,0]+0
        th_post_gam_tot_tab[nn,:,0,0] = t_tot[:,0]
        th_post_gam_tot_tab[nn,:,0,1] = t_tot[:,1]
    ...
    for nn,num in enumerate(num_est_tab) :
        curves_jeff_tmp = 1/2+1/2*spc.erf(np.log(rep0(ref.a_tab, nb_tot_HM)/rep1(th_post_jeff_tab[nn,:,0], ref.num_a))/rep1(th_post_jeff_tab[nn,:,1], ref.num_a))
        curves_gam_tmp = 1/2+1/2*spc.erf(np.log(rep0(ref.a_tab, nb_tot_HM)/rep1(th_post_gam_tab[nn,:,0], ref.num_a))/rep1(th_post_gam_tab[nn,:,1], ref.num_a))
        curves_MLE_tmp = 1/2+1/2*spc.erf(np.log(rep0(ref.a_tab, nb_MLE)/rep1(th_MLE_tab[nn,:,0], ref.num_a))/rep1(th_MLE_tab[nn,:,1], ref.num_a))
...
#todo : think about a sigma_prop
    ...
    return (err_conf_jeff_tab, err_med_jeff_tab, err_conf_gam_tab, err_med_gam_tab, err_conf_MLE_tab, err_med_MLE_tab)
...
err_conf_jeff_tab = np.zeros((cnb_tot,number_estimations))
err_med_jeff_tab = np.zeros((cnb_tot,number_estimations))
err_conf_gam_tab = np.zeros((cnb_tot,number_estimations))
err_med_gam_tab = np.zeros((cnb_tot,number_estimations))
err_conf_MLE_tab = np.zeros((cnb_tot,number_estimations))
err_med_MLE_tab = np.zeros((cnb_tot,number_estimations))



# cnb = 0

for cnb in range(cnb_tot) :

    S_tot, A_tot = get_S_A(path, IM, C, shuffle=True, relative=False)
    num_tot = A_tot.shape[0]

    err_conf_jeff_tab[cnb], err_med_jeff_tab[cnb], err_conf_gam_tab[cnb], err_med_gam_tab[cnb], err_conf_MLE_tab[cnb], err_med_MLE_tab[cnb] = total_conv_curves(S_tot, A_tot)
...
err_conf_jeff_tab, err_med_jeff_tab, err_conf_gam_tab, err_med_gam_tab, err_conf_MLE_tab, err_med_MLE_tab = err_conf_jeff_tab.mean(0), err_med_jeff_tab.mean(0), err_conf_gam_tab.mean(0), err_med_gam_tab.mean(0), err_conf_MLE_tab.mean(0), err_med_MLE_tab.mean(0)
...
f = open(path+'/for_figs/errors', 'wb')
pickle.dump((err_conf_jeff_tab, err_med_jeff_tab, err_conf_gam_tab, err_med_gam_tab, err_conf_MLE_tab, err_med_MLE_tab), f)
f.close()
